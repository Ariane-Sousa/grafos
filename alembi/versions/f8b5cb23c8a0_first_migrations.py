...
Revision ID: f8b5cb23c8a0
Revises: 
Create Date: 2024-06-02 01:14:21.584260
...
def upgrade() -> None:
    ...
    op.drop_index('idx_node_coordinates', table_name='node', postgresql_using='gist')
    ...
    op.drop_table('bg')
    op.drop_index('ix_graph_id', table_name='graph')
    ...
    op.drop_index('ix_edge_id', table_name='edge')
    op.drop_table('edge')
...
def downgrade() -> None:
    ...
    sa.Column('from_node_id', sa.INTEGER(), autoincrement=False, nullable=True),
    sa.Column('to_node_id', sa.INTEGER(), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['from_node_id'], ['node.id'], name='edge_from_node_id_fkey'),
    sa.ForeignKeyConstraint(['graph_id'], ['graph.id'], name='edge_graph_id_fkey'),
    sa.ForeignKeyConstraint(['to_node_id'], ['node.id'], name='edge_to_node_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='edge_pkey')
    )
    op.create_index('ix_edge_id', 'edge', ['id'], unique=False)
    ...
    sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('graph_id_seq'::regclass)"), autoincrement=True, nullable=False),
    sa.Column('name', sa.VARCHAR(), autoincrement=False, nullable=True),
    sa.PrimaryKeyConstraint('id', name='graph_pkey'),
    postgresql_ignore_search_path=False
    )
    op.create_index('ix_graph_name', 'graph', ['name'], unique=False)
    op.create_index('ix_graph_id', 'graph', ['id'], unique=False)
    op.create_table('bg',
    ...
    sa.ForeignKeyConstraint(['graph_id'], ['graph.id'], name='node_graph_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='node_pkey')
    )
    op.create_index('ix_node_id', 'node', ['id'], unique=False)
    op.create_index('idx_node_coordinates', 'node', ['coordinates'], unique=False, postgresql_using='gist')
